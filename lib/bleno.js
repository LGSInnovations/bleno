...
function Bleno(options) {
  ...
  if (bindings) {
    ...
    this._bindings = new Bindings(options);
  } else {
    throw new Error('Unsupported platform-based bindings');
  }
...
Bleno.prototype.startAdvertisingAltBeacon = function(manufacturerId, beaconId, referenceRssi, manufacturerReserved, callback) {
  if (this.state !== 'poweredOn') {
    var error = new Error('Could not start advertising, state is ' + this.state + ' (not poweredOn)');
    ...
    var altBeaconData = new Buffer(26);
    var undashedBeaconId = UuidUtil.removeDashes(beaconId);
    var beaconIdData = new Buffer(undashedBeaconId, 'hex');

    altBeaconData.writeUInt16BE(manufacturerId, 0);
    altBeaconData.writeUInt16BE(0xBEAC, 2); // AltBeacon ID code

    beaconIdData.copy(altBeaconData, 4);

    altBeaconData.writeUInt8(referenceRssi, 24);
    altBeaconData.writeUInt8(manufacturerReserved, 25);

    if (callback) {
      this.once('advertisingStart', callback);
    }

    debug('altBeaconData data = ' + altBeaconData.toString('hex'));

    this._bindings.startAdvertisingAltBeacon(altBeaconData);
  }
};
