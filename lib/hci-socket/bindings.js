...
var BlenoBindings = function(options) {
  options = options || {};
  ...
  var deviceName = options.deviceName ? options.deviceName : (process.env.BLENO_DEVICE_NAME || os.hostname());

  this._hci = new Hci();
  this._gap = new Gap(this._hci);
  this._gatt = new Gatt(deviceName);
...
BlenoBindings.prototype.startAdvertisingAltBeacon = function(data) {
  this._advertising = true;

  this._gap.startAdvertisingAltBeacon(data);
};
...
BlenoBindings.prototype.init = function (options) {
  options = options || {};

  var deviceId = options.deviceId ? options.deviceId : (process.env.BLENO_HCI_DEVICE_ID ? parseInt(process.env.BLENO_HCI_DEVICE_ID) : undefined);
  var channelUser = options.channelUser ? options.channelUser : (process.env.HCI_CHANNEL_USER ? process.env.HCI_CHANNEL_USER : undefined);
  var advertisementInterval = options.advertisementInterval ? options.advertisementInterval * 1.6 : Math.floor((process.env.BLENO_ADVERTISING_INTERVAL ? parseFloat(process.env.BLENO_ADVERTISING_INTERVAL) : 100) * 1.6);
  ...
  this._hci.init(deviceId, channelUser, advertisementInterval);
};
