...
Gap.prototype.startAdvertisingAltBeacon = function(data) {
  debug('startAdvertisingAltBeacon: data = ' + data.toString('hex'));

  var advertisementDataLength = 5 + data.length;
  var scanDataLength = 0;

  var advertisementData = new Buffer(advertisementDataLength);
  var scanData = new Buffer(scanDataLength);

  advertisementData.writeUInt8(0x02, 0); // Length
  advertisementData.writeUInt8(0x01, 1); // Data Type
  advertisementData.writeUInt8(0x06, 2); // EIR FLAGS - 0b00000110 = LE General Discoverable, BR/EDR not supported
  advertisementData.writeUInt8(advertisementDataLength + 1, 3); // AD LENGTH
  advertisementData.writeUInt8(0xff, 4); // AD TYPE

  data.copy(advertisementData, 5);
